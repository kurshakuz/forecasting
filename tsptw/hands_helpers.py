...
def generate_rectangles(num_regions, region_height_start, region_width, region_height):
    ...
    for i in range(num_regions*5):
        ...
        rectangles.append(InspectionSpace(rectangle_x, rectangle_y,
                          rectangle_width, rectangle_height, frame_id, region_id))
    ...
    for i, pair in enumerate(preds_pairs):
        ...
        if not (x_l == 0.0 or y_l == 0.0):
            ...
            if square_x_l+hand_size > 320*(i+1):
                square_width_l = min(
                    hand_size-0.01-(square_x_l+hand_size-320*(i+1)), hand_size)
            elif square_x_l < 320*i:
                square_width_l = 320*i - square_x_l
                square_x_l = 320*i + 0.01
            else:
                square_width_l = hand_size
            squares.append(InspectionSpace(square_x_l, square_y_l,
                           square_width_l, square_height, -1, -1))
        ...
        if not (x_r == 0.0 or y_r == 0.0):
            ...
            if square_x_r+hand_size > 320*(i+1):
                square_width_r = min(
                    hand_size-0.01-(square_x_r+hand_size-320*(i+1)), hand_size)
            elif square_x_r < 320*i:
                square_width_r = 320*i - square_x_r
                square_x_r = 320*i + 0.01
            else:
                square_width_r = hand_size
            squares.append(InspectionSpace(square_x_r, square_y_r,
                           square_width_r, square_height, -1, -1))
...
def define_regions_and_intersections(frame_width, num_regions, reg_preds_pairs):
    ...
    rectangles = generate_rectangles(
        num_regions, region_height_start, region_width, region_height)
    ...
    for i in range(5):
        ...
        for rectangle in rectangles[i*num_regions:(i+1)*num_regions]:
            ...
            if rectangle in intersecting:
                ...
                customers.append(
                    Customer(id, (center_x, center_y), 0*time_scaler, 0.5*time_scaler, 0.))
            id += 1
...
def load_customers_from_predictions(preds):
    ...
    rectangles, squares, intersecting = define_regions_and_intersections(
        frame_width, num_regions, reg_preds_pairs)
    instances = generate_tsptw_instances(
        frame_width, num_regions, rectangles, intersecting)

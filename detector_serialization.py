...
from model.utils.net_utils import save_net, load_net, vis_detections, vis_detections_PIL, vis_detections_filtered_objects_PIL, vis_detections_filtered_objects, filter_object, calculate_center
...
side_map3 = {0:'L', 1:'R'}
side_map4 = {0: HandSide.LEFT, 1: HandSide.RIGHT}
...
state_map3 = {0: HandState.NO_CONTACT, 1: HandState.SELF_CONTACT, 2: HandState.ANOTHER_PERSON, 3: HandState.PORTABLE_OBJECT, 4: HandState.STATIONARY_OBJECT}
...
def parse_args():
    ...
    parser.add_argument('--cfg', dest='cfg_file',
                        ...
                        default='detector/cfgs/res101.yml', type=str)
    ...
    parser.add_argument('--save_dir', dest='save_dir',
                        ...
                        default="images_det")
    parser.add_argument('--save_pkl_dir', dest='save_pkl_dir',
                    help='directory to save pkl results',
                    default="pkl_det")
    ...
    parser.add_argument('--thresh_obj', default=0.5,
                        type=float,
                        required=False)
    parser.add_argument('--get_pickle', dest='get_pickle',
                        help='get pickle mode',
                        default=True)
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        if webcam_num >= 0:
            ...
            imglist = sorted(os.listdir(image_dir), reverse=True)
        ...
        while (num_images >= 0):
            ...
            if vis and webcam_num == -1:
                ...
                # saving pickled detections
                else:
                    # print("Saving pickle\n")
                    hand_det_ser = []
                    object_det_ser = []

                    # case with both object and hand detections
                    if (obj_dets is not None) and (hand_dets is not None):
                        ...
                        for obj_idx, i in enumerate(range(np.minimum(10, obj_dets.shape[0]))):
                            ...
                            if score > thresh_obj and i in img_obj_id:
                                object_det_ser.append(ObjectDetection(
                        ...
                        for hand_idx, i in enumerate(range(np.minimum(10, hand_dets.shape[0]))):
                            ...
                            if score > thresh_hand:
                                if state > 0: # in contact hand
                                    ...
                                    hand_det_ser.append(HandDetection(
                                        bbox=BBox(bbox[0], bbox[1], bbox[2], bbox[3]),
                                        score=score,
                                        state=state_map3[state],
                                        side=side_map4[lr],
                                        object_offset=FloatVector(x=(hand_cc[0]-obj_cc[0]), y=(hand_cc[1]-obj_cc[1])),
                                    ))
                                else:
                                    # no contact
                                    obj_cc, hand_cc =  calculate_center(obj_dets[img_obj_id[i],:4]), calculate_center(bbox)
                        ...
                        for hand_idx, i in enumerate(range(np.minimum(10, hand_dets.shape[0]))):
                            ...
                            if score > thresh_hand:
                                hand_det_ser.append(HandDetection(
                                    ...
                                    state=state_map3[state],
                                    side=side_map4[lr],
                                    object_offset=FloatVector(x=0.0, y=0.0),
                                ))

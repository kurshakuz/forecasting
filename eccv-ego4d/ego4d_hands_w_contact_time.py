        ...
        for clip_id, hand_dicts in data.items():
            for hand_annot in hand_dicts:
                ...
                for annot in hand_annot['frames']:
                    ...
                    x = os.path.join(self.data_path, 'cropped_videos_ant', clip_name + '.mp4')
                    # print(x)
                    assert os.path.exists(x)
                    self._path_to_ant_videos.append(
                        os.path.join(self.data_path, 'cropped_videos_ant', clip_name + '.mp4')
                    )
                    ...
                    if contact_time != -1.0:
                        ...
                        self._labels.append(torch.Tensor([0.0]))
                        self._labels_masks.append(torch.Tensor([0.0]))
